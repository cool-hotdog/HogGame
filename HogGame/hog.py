...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    player_score_single = player_score % 10
    opponent_score_double = (opponent_score // 10) % 10
    diff = abs(player_score_single - opponent_score_double)
    return max(3 * diff,1)
    # END PROBLEM 2
...
def make_averaged(original_function, times_called=1000):
    ...
    def averaged_dice(*args):
        ...
        for _ in range(times_called):
            nums_list.append(original_function(*args))
        return sum(nums_list) / times_called
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for i in range(1,11):
        ...
        if i == 1:
            max_avg = averaged_roll(i, dice)
            max_num_rolls = i
        else:
            current_avg = averaged_roll(i, dice)
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    new_score = score + boar_brawl(score, opponent_score)
